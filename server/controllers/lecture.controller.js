...
export const uploadLecture = async (req, res) => {
  try {
    ...
    console.log('Upload lecture request:', { teacherId, title, subject, topic });
    ...
    console.log('Found teacher:', { teacherId, teacher: teacher ? { id: teacher._id, name: teacher.name, role: teacher.role } : null });
    
    if (!teacher || teacher.role !== 'teacher') {
      console.log('Role check failed:', { hasTeacher: !!teacher, role: teacher?.role });
      return res.status(403).json({
        success: false,
        message: "Only teachers can upload lectures"
      });
    }
